...
fn solution(input: Vec<&str>) -> u64 {
    ...
    for points in trail_start_points {
        let mut visited_top: HashSet<(i64, i64)> = HashSet::new();
        let value = count_reach_end(&mut grid, points, 0, &mut visited_top);
        amount += value;
    }
    amount
}
...
fn count_reach_end(
    ...
    expected: i64,
    visited_top: &mut HashSet<(i64, i64)>,
) -> u64 {
    ...
    if current == 9 && expected == 9 {
        if !visited_top.insert(current_position) {
            return 0;
        }
        return 1;
    }
    ...
    for dir in DIRECTIONS {
        ...
        count += count_reach_end(grid, next, expected + 1, visited_top);
    }
